...
static esp_err_t cops_read_parser(const char* response, void* parsed_data) {
    ...
    int matched = sscanf(start, "%d,%d,\"%63[^\"]\",%d",
...
static esp_err_t cops_write_formatter(const void* params, char* buffer, size_t buffer_size) {
    ...
    return snprintf(buffer, buffer_size, "%d,%d,\"%s\",%d",
        cops_params->mode,
        cops_params->format,
...
const at_cmd_t AT_CMD_COPS = {
    ...
    .type_info = {
        ...
        [AT_CMD_TYPE_READ] = {
            .parser = cops_read_parser,
            .formatter = NULL
        },
        [AT_CMD_TYPE_WRITE] = {
            .parser = NULL,  // Write command just expects OK or ERROR
