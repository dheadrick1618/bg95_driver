...
static esp_err_t cpin_write_formatter(const void* params, char* buffer, size_t buffer_size) {
    ...
    if (write_params->has_new_pin) {
        return snprintf(buffer, buffer_size, "=\"%s\",\"%s\"", 
                       write_params->pin, write_params->new_pin);
    } else {
...
const at_cmd_t AT_CMD_CPIN = {
    ...
    .type_info = {
        [AT_CMD_TYPE_TEST] = {
            .parser = NULL,  // Test command just returns OK
            .formatter = NULL
        },
        [AT_CMD_TYPE_READ] = {
            .parser = cpin_read_parser,
            .formatter = NULL
        },
        [AT_CMD_TYPE_WRITE] = {
            .parser = NULL,  // Write command expects OK or ERROR
            .formatter = cpin_write_formatter
        }
    },
    .timeout_ms = 5000,     // 5 seconds per spec
    .retry_count = 1        // No retries for PIN operations
};
