...
static esp_err_t uart_hw_write_impl(const char* data, size_t len, void* context) {
    ...
    if (available < len) {
        ESP_LOGW(TAG, "TX buffer doesn't have enough space. Available: %d, Need: %d. Flushing...", 
...
static esp_err_t uart_hw_read_impl(char* buffer, size_t max_len, size_t* bytes_read, 
    ...
    *bytes_read = 0;
    ...
    int len = uart_read_bytes(interface->uart_num, (uint8_t*)buffer, 
                            max_len - 1, ticks);
    if (len < 0) {
        return ESP_FAIL;
    }
...
esp_err_t bg95_uart_interface_init_hw(bg95_uart_interface_t* interface, bg95_uart_config_t config){
    ...
    ESP_LOGI(TAG, "Initializing UART interface with params: \nport %d\ntx pin: %d\nrx pin: %d", config.port_num, config.tx_gpio_num, config.rx_gpio_num);
  ...
  esp_err_t err = uart_driver_install(
      (uart_port_t) config.port_num, 
        BG95_UART_BUFF_SIZE * 2, 
        BG95_UART_BUFF_SIZE * 2, 
        0, 
        NULL, 
        0);
...
esp_err_t bg95_uart_interface_loopback_test(bg95_uart_interface_t* interface) {
    ...
    if (bytes_read != strlen(test_str) || 
        memcmp(test_str, rx_buffer, strlen(test_str)) != 0) {
